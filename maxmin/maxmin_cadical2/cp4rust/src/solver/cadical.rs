...
impl CaDiCaLSolver {
    ...
    /// This is mainly for compatibility with other solver backends
    pub fn set_num_vars(&mut self, _n: i32) {
        // CaDiCaL manages variables automatically, no explicit setting needed
    }

    /// Enumerate all solutions using blocking clauses
    /// Returns a vector of solutions, where each solution is a vector of booleans
    /// for each variable (index corresponds to variable ID: 1, 2, 3, ...)
    pub fn all_solutions(&mut self) -> Vec<Vec<bool>> {
        let mut solutions = Vec::new();
        let num_vars = self.next_var - 1;  // Number of variables we created

        if num_vars == 0 {
            return solutions;
        }

        // Mapping from variable ID to Bool for reconstruction
        let _var_to_bool: HashMap<i32, Bool> = self
            .bool_to_var
            .iter()
            .map(|(b, &var)| (var, b.clone()))
            .collect();

        // Keep solving with blocking clauses
        loop {
            ...
            let mut solution = vec![false; (num_vars + 1) as usize];  // Index 0 unused, 1..num_vars used
            let mut blocking_clause = Vec::new();

            for var_id in 1..=num_vars {
                let val = self.solver.val(var_id);
                let is_true = val > 0;
                solution[var_id as usize] = is_true;

                // Add opposite literal to blocking clause
                let blocking_lit = if is_true { -var_id } else { var_id };
                blocking_clause.push(blocking_lit);
            }

            // Store solution (skip index 0)
            solutions.push(solution[1..].to_vec());

            // Add blocking clause to prevent finding this solution again
    ...
    /// Add a clause directly (for blocking clauses in all_solutions)
    /// Uses raw variable IDs (1-based)
