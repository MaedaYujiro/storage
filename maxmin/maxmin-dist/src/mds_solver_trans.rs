...
use cp4rust::{BddMinisatSolver, Bool, Constraint, Encoder, CSP};
...
impl MdsSolverTrans {
    ...
    pub fn enumerate_minimal(&self) -> (Vec<Vec<usize>>, (i32, i32), (i32, i32)) {
        ...
        // Constraint 2: Minimality constraints
        // For each vertex v: CC(x_v) → ¬x_v
        // CC(x_v) = clauses containing x_v with x_v removed
        // These are the domination clauses for v and its neighbors, with x_v removed
        for v in 0..self.graph.num_vertices {
            // CC(x_v): For each clause that contains x_v, remove x_v from it
            // x_v appears in:
            //   - The domination clause for v: x_v ∨ (∨_{u ∈ N(v)} x_u)
            //     After removing x_v: (∨_{u ∈ N(v)} x_u)
            //   - The domination clause for each neighbor u of v: x_u ∨ (∨_{w ∈ N(u)} x_w)
            //     After removing x_v: x_u ∨ (∨_{w ∈ N(u), w ≠ v} x_w)
            ...
            if !neighbors_v.is_empty() {
                ...
                // v has no neighbors, so CC(x_v) from this clause is empty (always false)
                // This means the implication CC(x_v) → ¬x_v is always true when v is isolated
                // But actually, an isolated vertex must be in any dominating set
                // Skip adding constraint for isolated vertices
                continue;
            }
        ...
        // Original CSP statistics (as lower bound approximation)
        let orig_vars = self.graph.num_vertices as i32;
        let orig_clauses = (self.graph.num_vertices
            + self
                .graph
                .adj_list
                .iter()
